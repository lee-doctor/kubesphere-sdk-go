/*
KubeSphere

KubeSphere OpenAPI

API version: v3.1.0
Contact: info@kubesphere.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the DevopsNodeStepsActions type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &DevopsNodeStepsActions{}

// DevopsNodeStepsActions struct for DevopsNodeStepsActions
type DevopsNodeStepsActions struct {
	// It’s a fully qualified name and is an identifier of the producer of this resource's capability.
	Class *string `json:"_class,omitempty"`
	Links *Links `json:"_links,omitempty"`
	// url name
	UrlName *string `json:"urlName,omitempty"`
}

// NewDevopsNodeStepsActions instantiates a new DevopsNodeStepsActions object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewDevopsNodeStepsActions() *DevopsNodeStepsActions {
	this := DevopsNodeStepsActions{}
	return &this
}

// NewDevopsNodeStepsActionsWithDefaults instantiates a new DevopsNodeStepsActions object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewDevopsNodeStepsActionsWithDefaults() *DevopsNodeStepsActions {
	this := DevopsNodeStepsActions{}
	return &this
}

// GetClass returns the Class field value if set, zero value otherwise.
func (o *DevopsNodeStepsActions) GetClass() string {
	if o == nil || IsNil(o.Class) {
		var ret string
		return ret
	}
	return *o.Class
}

// GetClassOk returns a tuple with the Class field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DevopsNodeStepsActions) GetClassOk() (*string, bool) {
	if o == nil || IsNil(o.Class) {
		return nil, false
	}
	return o.Class, true
}

// HasClass returns a boolean if a field has been set.
func (o *DevopsNodeStepsActions) HasClass() bool {
	if o != nil && !IsNil(o.Class) {
		return true
	}

	return false
}

// SetClass gets a reference to the given string and assigns it to the Class field.
func (o *DevopsNodeStepsActions) SetClass(v string) {
	o.Class = &v
}

// GetLinks returns the Links field value if set, zero value otherwise.
func (o *DevopsNodeStepsActions) GetLinks() Links {
	if o == nil || IsNil(o.Links) {
		var ret Links
		return ret
	}
	return *o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DevopsNodeStepsActions) GetLinksOk() (*Links, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *DevopsNodeStepsActions) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given Links and assigns it to the Links field.
func (o *DevopsNodeStepsActions) SetLinks(v Links) {
	o.Links = &v
}

// GetUrlName returns the UrlName field value if set, zero value otherwise.
func (o *DevopsNodeStepsActions) GetUrlName() string {
	if o == nil || IsNil(o.UrlName) {
		var ret string
		return ret
	}
	return *o.UrlName
}

// GetUrlNameOk returns a tuple with the UrlName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *DevopsNodeStepsActions) GetUrlNameOk() (*string, bool) {
	if o == nil || IsNil(o.UrlName) {
		return nil, false
	}
	return o.UrlName, true
}

// HasUrlName returns a boolean if a field has been set.
func (o *DevopsNodeStepsActions) HasUrlName() bool {
	if o != nil && !IsNil(o.UrlName) {
		return true
	}

	return false
}

// SetUrlName gets a reference to the given string and assigns it to the UrlName field.
func (o *DevopsNodeStepsActions) SetUrlName(v string) {
	o.UrlName = &v
}

func (o DevopsNodeStepsActions) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o DevopsNodeStepsActions) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Class) {
		toSerialize["_class"] = o.Class
	}
	if !IsNil(o.Links) {
		toSerialize["_links"] = o.Links
	}
	if !IsNil(o.UrlName) {
		toSerialize["urlName"] = o.UrlName
	}
	return toSerialize, nil
}

type NullableDevopsNodeStepsActions struct {
	value *DevopsNodeStepsActions
	isSet bool
}

func (v NullableDevopsNodeStepsActions) Get() *DevopsNodeStepsActions {
	return v.value
}

func (v *NullableDevopsNodeStepsActions) Set(val *DevopsNodeStepsActions) {
	v.value = val
	v.isSet = true
}

func (v NullableDevopsNodeStepsActions) IsSet() bool {
	return v.isSet
}

func (v *NullableDevopsNodeStepsActions) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDevopsNodeStepsActions(val *DevopsNodeStepsActions) *NullableDevopsNodeStepsActions {
	return &NullableDevopsNodeStepsActions{value: val, isSet: true}
}

func (v NullableDevopsNodeStepsActions) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDevopsNodeStepsActions) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


